...
function Get-NativeToolPath([string]$Name) {
    ...
    Say-Verbose "command here is $command"
    if ($null -eq $command) {
...
function Try-Extract-Dotnet-Package-With7Zip([string]$ZipPath, [string]$OutPath) {
    $sevenZipPath = Get-NativeToolPath -Name "7z.exe"
    ...
    try {
        ...
        $EntriesToUnpack = New-Object System.Collections.Generic.List[string]

        foreach ($entry in $Zip.Entries) {
            # 7z creates directories while extracting files, so directory entries
            # do not need to be passed explicitly.
            if ($entry.FullName.EndsWith("/") -or $entry.FullName.EndsWith("\")) {
                continue
            }

            # Preserve side-by-side installs: never extract content below a
            # versioned directory that is already present locally.
            $PathWithVersion = Get-Path-Prefix-With-Version $entry.FullName
            if (($null -ne $PathWithVersion) -and -not ($DirectoriesToUnpack -contains $PathWithVersion)) {
                continue
            }

            # Preserve -SkipNonVersionedFiles behavior for files such as dotnet.exe.
            $DestinationPath = Get-Absolute-Path $(Join-Path -Path $OutPath -ChildPath $entry.FullName)
            if (($null -eq $PathWithVersion) -and (-not $OverrideNonVersionedFiles) -and (Test-Path $DestinationPath)) {
                continue
            }

            $EntriesToUnpack.Add($entry.FullName)
        }
        ...
        $entryListPath = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath ([System.IO.Path]::GetRandomFileName())

        # A list file prevents hitting the Windows command-line-length limit.
        [System.IO.File]::WriteAllLines(
            $entryListPath,
            [string[]]$EntriesToUnpack,
        ...
        if ($LASTEXITCODE -ne 0) {
            $sevenZipOutputText = ($sevenZipOutput | Out-String).Trim()

            if ([string]::IsNullOrWhiteSpace($sevenZipOutputText)) {
                throw "7z.exe extraction failed with exit code $LASTEXITCODE."
            }

            throw "7z.exe extraction failed with exit code $LASTEXITCODE. 7z output: $sevenZipOutputText"
        }
        ...
        if ($entryListPath -and (Test-Path $entryListPath)) {
            Remove-Item -Path $entryListPath -Force -ErrorAction SilentlyContinue
        }
    }
}
...
function Extract-Dotnet-Package([string]$ZipPath, [string]$OutPath) {
    ...
    if (Try-Extract-Dotnet-Package-With7Zip -ZipPath $ZipPath -OutPath $OutPath) {
        return
    }

    Say-Verbose "Extracting ZIP package with the managed extractor."
    Load-Assembly -Assembly System.IO.Compression.FileSystem
...
function DownloadFile-UsingHttpClient($Source, [string]$OutPath) {
    $Stream = $null
...
function Try-DownloadFile-WithCurl($Source, [string]$OutPath) {
    $curlPath = Get-NativeToolPath -Name "curl.exe"
    ...
    try {
        ...
        if (-not [string]::IsNullOrEmpty($ProxyAddress)) {
            $curlArguments += "--proxy"
            $curlArguments += $ProxyAddress
        }

        if ($ProxyBypassList.Count -gt 0) {
            $curlArguments += "--noproxy"
            $curlArguments += ($ProxyBypassList -join ",")
        }

        # Append the credential only at execution time. Do not log this value:
        ...
        if ($LASTEXITCODE -ne 0) {
            $curlOutputText = ($curlOutput | Out-String).Trim()

            if ([string]::IsNullOrWhiteSpace($curlOutputText)) {
                throw "curl.exe failed with exit code $LASTEXITCODE."
            }

            throw "curl.exe failed with exit code $LASTEXITCODE. curl output: $curlOutputText"
        }
...
function DownloadFile($Source, [string]$OutPath) {
    if ($Source -notlike "http*") {
        # Using System.IO.Path.GetFullPath does not work in this context;
        # $pwd supplies the current directory.
    ...
    if (Try-DownloadFile-WithCurl -Source $Source -OutPath $OutPath) {
