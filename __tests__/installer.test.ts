...
describe('installer tests', () => {
  ...
  describe('DotnetCoreInstaller tests', () => {
    ...
    describe('installDotnet() tests', () => {
      ...
      it(`should not supply 'architecture' argument when it isn't set (preserve installer auto-detect)`, async () => {
        const inputVersion = '10.0.101';
        const inputQuality = '' as QualityOptions;
        const stdout = `Fictitious dotnet version ${inputVersion} is installed`;
        ...
        // We validate both calls: runtime install (0) and SDK install (1)
        const runtimeArgs = (
        ...
        const archFlag = IS_WINDOWS ? `-Architecture` : `--architecture`;

        expect(runtimeArgs).not.toContain(archFlag);
        expect(sdkArgs).not.toContain(archFlag);
      });

      each(['x64', 'arm64']).test(
        `should supply 'architecture' argument to the installation script when it is set (%s)`,
        async inputArchitecture => {
          const inputVersion = '10.0.101';
          const inputQuality = '' as QualityOptions;
          const stdout = `Fictitious dotnet version ${inputVersion} is installed`;
          ...
          expect(runtimeArgs).toContain(expectedArgument);
          expect(sdkArgs).toContain(expectedArgument);
        }
      );
